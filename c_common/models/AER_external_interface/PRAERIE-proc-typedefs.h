...
typedef struct spike_link_t {
    ...
    uchar next; // index to the next element
} spike_link_t;

// this is the entire linked list
typedef struct spike_ll_t {
    spike_link_t spikes[SPIKE_BUF_MAX_SPIKES];
    uchar first; // the first event in the linked list.
    uchar last;  // the last event in the linked list.
    uchar end;  // index of the final (free) element. If last == end the buffer is full.
} spike_ll_t;
