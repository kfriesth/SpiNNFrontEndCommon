...
{return spike_buf->spike_list.spikes[spike_buf->spike_list.last].spike;}

static inline praerie_event_t earliest_spike(spike_buf_t* spike_buf)
{return spike_buf->spike_list.spikes[spike_buf->spike_list.first].spike;}
...
{
     // set up the status indices into the linked list
     buf->first = 0;
     buf->last = SPIKE_BUF_MAX_SPIKES; // indicates empty buffer
     buf->end = SPIKE_BUF_MAX_SPIKES-1;
     // then initialise the linked-list indices.
     for (uint event = 0; event < SPIKE_BUF_MAX_SPIKES-1; event++)
     {
         buf->spikes[event].next = event+1;
     }
     buf->spikes[SPIKE_BUF_MAX_SPIKES-1].next = buf->first;
}
...
static bool buffer_init(uint32_t* payload_sz)
{
     ...
     for (uint32_t with_payload = NO_PAYLOAD; with_payload <= WITH_PAYLOAD; with_payload++)
     {
         if (payload_sz[with_payload]) 
         {
            ...
            spike_ll_init(&immediate_inject_buf[with_payload]->spike_list);
...
{return buf->last == buf->end;}

// 1-beyond-maximum-size index for last indicates an empty buffer
static inline bool dma_buf_empty(spike_ll_t* buf) 
{return buf->last == SPIKE_BUF_MAX_SPIKES;}
...
{
     // may need the following test to filter out empty buffers
     // if dma_buf_empty(&comp_buf->spike_list) return false;
...
/*
   ...
   buffer full status: -1 - buffer was filled by this insert; 0 - buffer was
   already full; 1 - buffer not full.
*/
static signed char dma_buf_insert(spike_ll_t* buf, praerie_event_t event)
{
       // exit if the buffer is full 
       if (dma_buf_full(buf)) return 0;
       // initialise to the first available entry
       uchar new_spike_idx = dma_buf_empty(buf) ? buf->first : buf->spikes[buf->last].next;
       // and insert the new event
       buf->spikes[new_spike_idx].spike = event; 
       // most common case: timestamp is at least as late as the latest.
       // insert at the end.
       if (dma_buf_empty(buf) || ((event.timestamp - buf->spikes[buf->last].spike.timestamp) < DMA_IN_TIMESTAMP_HORIZON))
	  buf->last = new_spike_idx; 
       else 
       {
          // another simple case: timestamp is at least as early as the earliest.
          if ((buf->spikes[buf->first].spike.timestamp - event.timestamp) < DMA_IN_TIMESTAMP_HORIZON)
             buf->first = new_spike_idx; 
	  // start with an insert point at the very top of the buffer. 
	  uchar spike1_idx = buf->end;
	  // will need to search through the list.
          while ((event.timestamp - buf->spikes[buf->spikes[spike1_idx].next].spike.timestamp) < DMA_IN_TIMESTAMP_HORIZON)
             spike1_idx = buf->spikes[spike1_idx].next;
          // once we have reached the insert position update the indices
          buf->spikes[buf->last].next = buf->spikes[new_spike_idx].next;
          buf->spikes[new_spike_idx].next = buf->spikes[spike1_idx].next;
          buf->spikes[spike1_idx].next = new_spike_idx;
       }
       return dma_buf_full(buf) ? -1 : 1;
}				
...
static bool dma_buf_update(praerie_event_t* spike, spike_buf_t* comp_buf, spike_buf_t* curr_buf, spike_buf_t* next_buf, uint32_t with_payload, t_comp comp, bool latest)
{
     ...
     if (dma_buf_insert(&curr_buf->spike_list, *spike) < 0)
     {
...
static praerie_event_t dma_buf_remove(spike_ll_t* buf)
{
       ...
       uchar event_idx = buf->first;
       // if the list is empty, invalidate the returned event by giving
       // it a timestamp before the current local time (which means it will 
       // be discarded)
       if (dma_buf_empty(buf)) buf->spikes[event_idx].spike.timestamp = local_timestamp-1;
       else
       {
	  // this was the last entry. Set the last index to 1-beyond max to indicate empty.
          if (event_idx == buf->last) buf->last = SPIKE_BUF_MAX_SPIKES;
	  // otherwise read just the linked-list pointers by inserting the
          // newly-freed entry into the back of the available entries.
          else
          {
	     // event had been the first entry so new first is its next. 
             buf->first = buf->spikes[event_idx].next;
             // first set old end to point to newly freed event, then set end
             // to be that event.
             buf->end = buf->spikes[buf->end].next = event_idx;
          }
       }
       // and return the event
       return buf->spikes[event_idx].spike;
}
...
static uint32_t inject_spikes(spike_ll_t* spike_buffer, uint32_t with_payload)
{
         ...
         while (!(dma_buf_empty(spike_buffer)) && (spike_buffer->spikes[spike_buffer->first].spike.timestamp <= local_timestamp))
	 {
               // dequeue the event
                  ...
                  if (retry_count <= NUM_MC_SEND_RETRIES)
		  {
                  ...
                  else
	          {
	             ...
	             if (!dma_buf_insert(spike_buffer, spike))
#ifdef INSTRUMENTATION
...
{
       if ((src_data->transfer_ID = spin1_dma_transfer(src_data->tag, dst_addr, &src_data->spike_list, DMA_WRITE, sizeof(spike_ll_t))) == FAILURE) return (spin1_schedule_callback(_initiate_manual_write, (intptr_t)src_data, (intptr_t)dst_addr, CB_PRIORITY_MANUAL_XFER) == SUCCESS);
       else return true;
}

static inline bool initiate_dma_read(spike_buf_t* dst_data, void* src_addr)
{
       if ((dst_data->transfer_ID = spin1_dma_transfer(dst_data->tag, src_addr, &dst_data->spike_list, DMA_READ, sizeof(spike_ll_t))) == FAILURE) return (spin1_schedule_callback(_initiate_manual_read, (intptr_t)src_addr, (intptr_t)dst_data, CB_PRIORITY_MANUAL_XFER) == SUCCESS);
       else return true;
}
...
/* buffer insertion and reordering into the DMA buffers. This is complicated by
   ...
   transitory situations that can arise, mostly if there is a sudden input 
...
static bool dma_buf_add_spike(praerie_event_t* spike) 
{
     ...
     if (spike_time == local_timestamp)
     {
        ...
        while (!dma_buf_insert(&immediate_inject_buf[has_pyld]->spike_list, *spike))
	{
	   // so send the spikes now and then reinsert. 
#ifdef INSTRUMENTATION
           incr_prov(ABANDONED_SPIKES+has_pyld, SPIKE_BUF_MAX_SPIKES - inject_spikes(&immediate_inject_buf[has_pyld]->spike_list, has_pyld));
#else
           uint dropped_spikes = SPIKE_BUF_MAX_SPIKES - inject_spikes(&immediate_inject_buf[has_pyld]->spike_list, has_pyld);
           if (dropped_spikes)
...
static void buffer_or_send_new_spikes(void)
{
       ...
       inject_spikes(&immediate_inject_buf[NO_PAYLOAD]->spike_list, NO_PAYLOAD);
       inject_spikes(&immediate_inject_buf[WITH_PAYLOAD]->spike_list, WITH_PAYLOAD);
}
...
static void dma_writeback_done(uint8_t with_payload)
{
     ...
     if ((xfer_buf = read_pipeline[DMA_WBUF][with_payload])->tag & DMA_TAG_FULL)
     {
	   ...
	   // if so, make it the next one eligible for writeback  
        ...
        if ((sdram_w_buf[with_payload]++) >= sdram_end_buf[with_payload]) sdram_w_buf[with_payload] = sdram_evt_buf[with_payload];
...
static void dma_readback_done(uint transfer_id, uint8_t with_payload)
{
     ...
     { 
        ...
        if ((sdram_r_buf[with_payload]++) >= sdram_end_buf[with_payload]) sdram_r_buf[with_payload] = sdram_evt_buf[with_payload];
...
void send_old_spikes(void)
{
       ...
       for (uint8_t with_payload = NO_PAYLOAD; with_payload <= WITH_PAYLOAD; with_payload++)
       {
           ...
           while (inject_spikes(&(early_buf_orig = read_pipeline[EARLY_BUF][with_payload])->spike_list, with_payload) > 0)
           {
                 // if this exhausts the buffer and there's nothing left to send
                 // revert to expecting fills in the early buffer 
                 if ((dma_buf_empty(&early_buf_orig->spike_list)) ||
...
{
     uint32_t blk_count[2] = {no_payload_blk_count, with_payload_blk_count};
...
void _initiate_manual_write(intptr_t src, intptr_t dst)
{
     ...
     spin1_memcpy(sdram_buf, &local_buf->spike_list, sizeof(spike_ll_t));
...
void _initiate_manual_read(intptr_t src, intptr_t dst)
{
     ...
     spin1_memcpy(&local_buf->spike_list, sdram, sizeof(spike_ll_t));
     spin1_schedule_callback(_dma_done_callback, FAILURE, local_buf->tag, CB_PRIORITY_DMA_DONE);
#else
     spin1_delay_us(DMA_MANUAL_RETRY_DELAY);
