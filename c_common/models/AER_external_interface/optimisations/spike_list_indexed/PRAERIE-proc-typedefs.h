...
*/
// This defines a link type which allows referencing events by
// index rather than pointer.
typedef struct spike_link_t {
    uchar first; // the first event in the linked list. Only valid for entry 0. 
    uchar last;  // the last event in the linked list. Only valid for entry 0.
    uchar next;  // index of the next element. 
    uchar prev;  // index of the previous element. Or count for first element.
} spike_link_t;

// this is an element with its associated index structure
...
typedef struct spike_buf_t {
    ...
    uint tag;         // DMA tag for buffer. Will be last seq. 0xFFFFFFFF = free 
    uint transfer_ID; // DMA TID. Only assigned when a DMA is scheduled.
    spike_ll_t spikes[SPIKE_BUF_MAX_SPIKES]; // the actual spikes in the buffer
} spike_buf_t;
