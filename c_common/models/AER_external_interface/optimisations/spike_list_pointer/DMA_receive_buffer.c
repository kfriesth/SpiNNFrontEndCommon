...
{return spike_buf->spike_list.status.last->spike;}

static inline praerie_event_t earliest_spike(spike_buf_t* spike_buf)
{return spike_buf->spike_list.status.first->spike;}
...
static void spike_ll_init(spike_ll_t* buf)
{
     ...
     buf->status.first = &buf->spikes[0];
     buf->status.last = NULL;
     buf->status.end = &buf->spikes[SPIKE_BUF_MAX_SPIKES-1];
     // then initialise the linked-list indices.
     for (uint event = 0; event < SPIKE_BUF_MAX_SPIKES-1; event++)
     {
         buf->spikes[event].next = &buf->spikes[event+1];
     }
     buf->spikes[SPIKE_BUF_MAX_SPIKES-1].next = buf->status.first;
}
...
{return buf->status.last == buf->status.end;}

static inline bool dma_buf_empty(spike_ll_t* buf) 
{return buf->status.last == NULL;}
...
*/
static int8_t dma_buf_insert(spike_ll_t* buf, praerie_event_t event)
{
       // exit if the buffer is full 
       if (dma_buf_full(buf)) return 0;
       // initialise to the first available entry
       spike_link_t* new_spike = dma_buf_empty(buf) ? buf->status.first : buf->status.last->next;
       // and insert the new event
       new_spike->spike = event; 
       // most common case: timestamp is at least as late as the latest.
       // insert at the end.
       if (dma_buf_empty(buf) || ((event.timestamp - buf->status.last->spike.timestamp) < DMA_IN_TIMESTAMP_HORIZON))
	  buf->status.last = new_spike; 
       else 
       {
          // another simple case: timestamp is at least as early as the earliest.
          if ((buf->status.first->spike.timestamp - event.timestamp) < DMA_IN_TIMESTAMP_HORIZON)
             buf->status.first = new_spike; 
	  // start with an insert point at the very top of the buffer. 
	  spike_link_t* spike1 = buf->status.end;
	  // will need to search through the list.
          while ((event.timestamp - spike1->next->spike.timestamp) < DMA_IN_TIMESTAMP_HORIZON)
	        spike1 = spike1->next;
          // once we have reached the insert position update the indices
          buf->status.last->next = new_spike->next;
          new_spike->next = spike1->next;
          spike1->next = new_spike;
       }
...
static praerie_event_t dma_buf_remove(spike_ll_t* buf)
{
       ...
       spike_link_t* event = buf->status.first;
       ...
       // be discarded)
       if (dma_buf_empty(buf)) event->spike.timestamp = local_timestamp-1;
       else
       {
	  // this was the last entry. Clear the last pointer to indicate empty.
          if (event == buf->status.last) buf->status.last = NULL;
	  // otherwise read just the linked-list pointers by inserting the
	     ...
	     // event had been the first entry so new first is its next. 
             buf->status.first = event->next;
             // first set old end to point to newly freed event, then set end
             // to be that event.
             buf->status.end = buf->status.end->next = event;
          }
       }
...
static uint32_t inject_spikes(spike_ll_t* spike_buffer, uint32_t with_payload)
{
         ...
         while (!(dma_buf_empty(spike_buffer)) && (spike_buffer->status.first->spike.timestamp <= local_timestamp))
	 {
               // dequeue the event
